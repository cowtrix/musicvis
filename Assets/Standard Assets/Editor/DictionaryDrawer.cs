...
{
    private CompositionDictionary<TK, TV> _Dictionary;
    private bool _Foldout;
    private const float kButtonWidth = 18f;

    public virtual float? KeyWidth {get { return null; }}
    public virtual float? ValueWidth { get { return null; } }
    public virtual float? Height {get { return null; }}

    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        CheckInitialize(property, label);
        if (_Foldout)
            return (_Dictionary.Count + 1) * (Height.HasValue ? Height.Value : 17f);
        return 17f;
    }
    ...
    {
        if (EditorUtility.DisplayDialog("Really clear dictionary?", "Yes", "No"))
        {
            _Dictionary.Clear();
        }
    }
    ...
    private void AddNewItem()
    {
        ...
        if (typeof(UnityEngine.ScriptableObject).IsAssignableFrom(typeof(TK)))
        {
            var sc = UnityEngine.ScriptableObject.CreateInstance(typeof(TK));
            key = (TK)(object)sc;
