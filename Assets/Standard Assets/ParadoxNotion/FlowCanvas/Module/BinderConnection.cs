	...
	public class BinderConnection : Connection {
		...
		public static BinderConnection Create(Port source, Port target){
			...
			if (!TypeConverter.HasConvertion(source.type, target.type)){
				Debug.LogWarning(string.Format("Can't connect ports. Type '{0}' is not assignable from Type '{1}' and there exists no automatic conversion for those types.", target.type.FriendlyName(), source.type.FriendlyName() ));
				return null;
			}
