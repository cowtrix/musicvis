...
namespace ParadoxNotion.Design{
    ...
    public static class UserTypePrefs {
		...
		public static List<Type> GetPreferedTypesList(Type baseType, bool filterOutFunctionalOnlyTypes = false){
			...
			foreach(var t in _preferedTypes){
				if (t.IsGenericType){
					continue;
				}
				if (filterOutFunctionalOnlyTypes && ( (t.IsAbstract && !t.IsInterface ) || filterFunctionalTypes.Contains(t)) ){
					continue;
				}
				if (baseType != typeof(object) && !baseType.IsAssignableFrom(t)){
					continue;
				}
				types.Add(t);
			}

			return types.OrderBy(t => t.Name).OrderBy(t => t.Namespace).ToList();
		}
		...
		public static void ResetTypeConfiguration(){
			EditorPrefs.SetString(typesPrefsKey, defaultTypesListString);
			_preferedTypes = null;
		}
		...
		private static Dictionary<Type, Color> typeColors = new Dictionary<Type, Color>()
		{
			...
			{typeof(string),             new Color(0.45f,0.45f,0.45f)},
		...
		public const string DEFAULT_TYPE_ICON_NAME = "DefaultTypeIcon";

		//A Type to texture lookup
		...
		public static Texture GetTypeIcon(Type type, bool fallbackToDefault = true){
			...
			if (texture == null){
				texture = Resources.Load<Texture>(type.FullName);
			}

			if (texture == null){
				var current = type.BaseType;
				while(current != null){
					texture = Resources.Load<Texture>(current.FullName);
			...
			if (texture == null){
				texture = Resources.Load<Texture>(DEFAULT_TYPE_ICON_NAME);
			}
