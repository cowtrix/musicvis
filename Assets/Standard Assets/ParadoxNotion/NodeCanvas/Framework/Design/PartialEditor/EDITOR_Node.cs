	...
	partial class Node{
		...
		void ShowHeader(){
			...
			if (!showIcon || iconAlignment != Alignment2x2.Default){
				if (name != null){
					if (!EditorGUIUtility.isProSkin){ //fix light theme coloring by adding a dark background
					...
					finalTitle = System.Text.RegularExpressions.Regex.Replace(finalTitle, "<color>", "(Color)", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
					var image = showIcon && iconAlignment == Alignment2x2.Left? icon : null;
					var text = string.Format("<b><size=12><color=#{0}>{1}{2}</color></size></b>", hexColor, (image != null? " " : ""), finalTitle);
