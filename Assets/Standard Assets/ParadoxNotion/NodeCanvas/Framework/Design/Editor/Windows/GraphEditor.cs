...
namespace NodeCanvas.Editor{
	...
	public class GraphEditor : EditorWindow{
		...
		readonly static string WATERMARK = "@NodeCanvas Framework v" + GraphSerializationData.FRAMEWORK_VERSION.ToString();
		...
		void OnGUI(){
			...
			canvasRect = Rect.MinMaxRect(5, TOP_MARGIN, position.width -5, position.height -5);
			...
			ShowScrollBars();
			ShowMiniMap(new Rect(100,100,300,300));
		...
		void FocusConnection(Connection connection){
			var mid = (( connection.sourceNode.nodeRect.center - connection.targetNode.nodeRect.center ) * 0.5f) + connection.targetNode.nodeRect.center;
			FocusPosition(mid);
		}
		...
		void ShowScrollBars(){
			...
			if (yTopDiff > 0 || yBottomDiff > 0){
				var yScrollBarRect = new Rect(position.width - 20, TOP_MARGIN, 10, position.height - TOP_MARGIN - 20);
		...
		void DrawGrid(Rect container, Vector2 offset, float zoomFactor){
			...
			// GUI.Box(container, offset.ToString());
			...
			for (var i = xStart; i < xEnd; i += step){
				var high = false;
				Handles.color = new Color(0,0,0, high? 0.5f : 0.1f);
		...
		Rect RectToRect(Rect rect, Rect rectContainer, Rect newContainer){
			rect.xMin = Mathf.LerpUnclamped(newContainer.xMin, newContainer.xMax, Mathf.InverseLerp(rect.xMin, rectContainer.xMin, rectContainer.xMax));
			rect.xMax = Mathf.LerpUnclamped(newContainer.xMin, newContainer.xMax, Mathf.InverseLerp(rect.xMax, rectContainer.xMin, rectContainer.xMax));
			rect.yMin = Mathf.LerpUnclamped(newContainer.yMin, newContainer.yMax, Mathf.InverseLerp(rect.yMin, rectContainer.yMin, rectContainer.yMax));
			rect.yMax = Mathf.LerpUnclamped(newContainer.yMin, newContainer.yMax, Mathf.InverseLerp(rect.yMax, rectContainer.yMin, rectContainer.yMax));
			return rect;
		}

		void ShowMiniMap(Rect container){
			//TODO
		}
