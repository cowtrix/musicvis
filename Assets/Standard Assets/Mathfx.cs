...
{
    /// <summary>
    ///     Floors the value, but with negative numbers floors as if they were positive (e.g -0.9 = 0)
    /// </summary>
    /// <param name="value"></param>
    /// <returns></returns>
    public static float FloorWithNegative(float value)
    {
        var sign = Mathf.Sign(value);
        value = Mathf.Floor(Math.Abs(value));
        return value * sign;
    }
    ...
    public static Vector3 QuakeClipInverse(Vector3 source, Vector3 normal, bool forwardOnly = false,
        float overbounce = 1.011f)
    {
        var backoff = Vector3.Dot(source, normal);
    ...
    public static Vector3 CalculateInterceptCourse(Vector3 aTargetPos, Vector3 aTargetSpeed, Vector3 aInterceptorPos,
        float aInterceptorSpeed, float maxLead = 9999f)
    {
        ...
        result.Normalize();
        return FindClosestPointOfApproach(aTargetPos, aTargetSpeed, aInterceptorPos, result * aInterceptorSpeed) *
               aTargetSpeed;
